...
public final class Settings {
    ...
    public void loadConfig() {
        ...
        title = ChatColor.translateAlternateColorCodes('&'
                , checkLength(config.getString("Scoreboard.Title"), 32));
        ...
        tempTitle = ChatColor.translateAlternateColorCodes('&',
                checkLength(config.getString("Temp-Scoreboard.Title"), 32));
    }
    ...
    private String checkLength(String check, int limit) {
        if (check.length() > limit) {
            ...
            pluginInstance.getLogger().warning("tooLongName");
    ...
    private int checkItems(int input) {
        if (input >= 16) {
            pluginInstance.getLogger().warning("tooManyItems");
            return 16 - 1;
        }

        if (input <= 0) {
            pluginInstance.getLogger().warning("notEnoughItems");
            return 5;
        }
    ...
    private void loaditems(ConfigurationSection config) {
        ...
        for (String key : keys) {
            ...
            final String name = ChatColor.translateAlternateColorCodes('&', checkLength(key, 16));
        ...
        if (ITEMS.isEmpty()) {
            pluginInstance.getLogger().info("notEnoughItems");
        }
    }
}