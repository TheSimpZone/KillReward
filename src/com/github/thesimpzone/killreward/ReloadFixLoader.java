...
public class ReloadFixLoader extends ClassLoader {
    ...
    public static boolean changeClassCache(boolean status) {
        try {
            ...
            //Logger.getLogger("KillReward").severe(Lang.get("changeChageEx", ex));
        	Logger.getLogger("KillReward").severe("Couldn't disable cache system for classes. This can cause some invalid initialization" + ex);
            return false;
        }
    }

    private final File pluginFile = KillReward.getInstance().getFileBypass();
    ...
    private void closeQuietly(JarFile file) {
        if (file != null) {
            try {
                ...
                Logger.getLogger("FileReward").log(Level.SEVERE, null, ex);
            }
        }
    }
}